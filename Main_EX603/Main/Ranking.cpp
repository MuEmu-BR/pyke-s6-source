...
void cRanking::Draw()
{
	...
	pDrawGUI(100037, StartX + 160, StartY + 55, 312, 292);
	//0x7EC5, 36, 29, 415, 30);
	pDrawGUI(0x7EC5, StartX + 435, StartY + 65, 36, 29);
	int TitleX = StartX + 180;
	...
	pDrawGUI(0x7BAA, StartX+60+ButX, StartY+225+ButY , 17, 17);
	
	if(gInterface.CursorZoneButton(StartX+60+ButX, StartY+225+ButY, StartX+60+17+ButX, StartY+225+17+ButY))
	{
		...
		pDrawColorButton(0x7BAA, StartX+60+ButX, StartY+225+ButY, 17, 17, 0, 0, pMakeColor(255, 204, 20, 130));
	}

	gInterface.DrawFormat(eShinyGreen, StartX + 96 + ButX, StartY + 227 + ButY, 100, 1, "Page: %d/%d", this->Page, 10);


	pDrawGUI(0x7BAB, StartX+150+ButX, StartY+225+ButY , 17, 17);

	if(gInterface.CursorZoneButton(StartX+150+ButX, StartY+225+ButY, StartX+150+17+ButX, StartY+225+17+ButY))
	{
		if(gController.wMouse == WM_LBUTTONDOWN && Delay > 300)
		{
			this->ClickTime = GetTickCount();
		...
		pDrawColorButton(0x7BAB, StartX+150+ButX, StartY+225+ButY, 17, 17, 0, 0, pMakeColor(255, 204, 20, 130));
	}
	//pDrawGUI(0x7EC5, StartX + 505, StartY + 83, 36, 29);
	if(gInterface.CursorZoneButton(StartX+435, StartY + 65, StartX + 460, StartY + 90))
	{
		if(gController.wMouse == WM_LBUTTONDOWN && Delay > 300)
		{
			this->ClickTime = GetTickCount();
		...
		pDrawColorButton(0x7EC5, StartX + 435, StartY + 65, 36, 29, 0, 0, pMakeColor(255, 204, 20, 130));
	}
}
