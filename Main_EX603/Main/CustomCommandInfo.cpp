...
void CCustomCommandInfo::Load(CUSTOM_COMMAND_INFO* info){
	for (int n = 0; n < MAX_CUSTOM_COMMAND; n++){
		this->SetInfo(info[n]);
	}
}

void CCustomCommandInfo::SetInfo(CUSTOM_COMMAND_INFO info){
...
CUSTOM_COMMAND_INFO* CCustomCommandInfo::GetInfo(int index){
...
void CCustomCommandInfo::DrawCommandWindow(){
	...
	for (int i = (25 * Page) + 0; i < (25 * Page) + 25; i++){
		...
		float postYCommand, postXDescription, postWidthCommand, postWidthDescription;
		postYCommand = (int)StartBody + 35 + (x * 10);
		postXDescription = (int)StartX + 112;
		postWidthCommand = 100;
		postWidthDescription = 100;
		gInterface.DrawFormat(Color, (int)StartX + 18, postYCommand, postWidthCommand, 1, this->m_CustomCommandInfo[i].Command);
		//gInterface.DrawFormat(Color, (int)StartX + 112, (int)StartBody + 35 + (i*10), 100, 1, this->m_CustomCommandInfo[i].Description);
		pDrawColorText(this->m_CustomCommandInfo[i].Description, postXDescription, postYCommand, postWidthDescription, 0, Color, 0, 1);
	}
}
