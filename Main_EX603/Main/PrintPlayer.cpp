...
void InitPrintPlayer() // OK
{
	...
	SetCompleteHook(0xE9, 0x0077FB09, &PrintPlayerLevel1); //0077FAE0
...
void PrintTextReplace(DWORD* value, DWORD type) // OK
{
	...
	case 206: //"Taxa de defesa: %d (%d +%d)"
		...
		value[3] = ViewDefenseSuccessRate / 10;
		////console.Log(FUNC, "%d %d %d %d", value[0], value[1], value[2], value[3]);
		break;
	case 207: //"Defesa: %d (+%d)"
		value[0] = 209;
		value[1] = ViewDefense;
...
void PrintFixStatPoint(DWORD type) // OK
{
	...
	case 0:
		...
		gObjUser.lpPlayer->Strength = GET_MAX_WORD_VALUE(ViewStrength);
		gObjUser.lpPlayer->AddStrength = GET_MAX_WORD_VALUE(ViewAddStrength);
		break;
	case 1:
		...
		gObjUser.lpPlayer->Dexterity = GET_MAX_WORD_VALUE(ViewDexterity);
		gObjUser.lpPlayer->AddDexterity = GET_MAX_WORD_VALUE(ViewAddDexterity);
		break;
	case 2:
		...
		gObjUser.lpPlayer->Vitality = GET_MAX_WORD_VALUE(ViewVitality);
		gObjUser.lpPlayer->AddVitality = GET_MAX_WORD_VALUE(ViewAddVitality);
		break;
	case 3:
		...
		gObjUser.lpPlayer->Energy = GET_MAX_WORD_VALUE(ViewEnergy);
		gObjUser.lpPlayer->AddEnergy = GET_MAX_WORD_VALUE(ViewAddEnergy);
		break;
	case 4:
		...
		gObjUser.lpPlayer->Leadership = GET_MAX_WORD_VALUE(ViewLeadership);
		gObjUser.lpPlayer->AddLeadership = GET_MAX_WORD_VALUE(ViewAddLeadership);
		break;
	}
}
...
__declspec(naked) void PrintPlayerLevel1() // OK
{
	...
	static DWORD MLevel = gObjUser.m_MasterLevel;
