...
public:
	...
	static BOOL LuaGlColor4f(lua_State * Lua)
	{
		const GLfloat Red = RGBTOGLFLOAT(lua_tointeger(Lua,1));
		const GLfloat Green = RGBTOGLFLOAT(lua_tointeger(Lua,2));
		const GLfloat Blue = RGBTOGLFLOAT(lua_tointeger(Lua,3));
		const GLfloat Alpha = RGBTOGLFLOAT(lua_tointeger(Lua,4));

		glColor4f(Red,Green,Blue,Alpha);
		//glColor4f(atof(lua_tostring(Lua,1)),atof(lua_tostring(Lua,2)),atof(lua_tostring(Lua,3)),atof(lua_tostring(Lua,4)));
		return true;
	}

	static BOOL LuaSetBkColor(lua_State * Lua/*BYTE red, BYTE green, BYTE blue, BYTE alpha*/)
	{
		//gConsole.AddMessage(5, "BK_COLOR [%x %x %x %x]", (BYTE)lua_tointeger(Lua, 1), (BYTE)lua_tointeger(Lua, 2), (BYTE)lua_tointeger(Lua, 3), (BYTE)lua_tointeger(Lua, 4));
	...
	{
		//gConsole.AddMessage(5, "TEXT_COLOR [%x %x %x %x]", (BYTE)lua_tointeger(Lua, 1), (BYTE)lua_tointeger(Lua, 2), (BYTE)lua_tointeger(Lua, 3), (BYTE)lua_tointeger(Lua, 4));
	...
	static BOOL LuaCreateFont(lua_State * Lua)
	{
		HFONT Font = CreateFontA(lua_tointeger(Lua,1), lua_tointeger(Lua,2), lua_tointeger(Lua,3), lua_tointeger(Lua,4), lua_tointeger(Lua,5),
			(DWORD)lua_tointeger(Lua,6), (DWORD)lua_tointeger(Lua,7), (DWORD)lua_tointeger(Lua,8), (DWORD)lua_tointeger(Lua,9), (DWORD)lua_tointeger(Lua,10), (DWORD)lua_tointeger(Lua,11),
			(DWORD)lua_tointeger(Lua,12), (DWORD)lua_tointeger(Lua,13), lua_tostring(Lua,14));
		lua_pushinteger(Lua, (DWORD)Font);
		return true;
	}

	static BOOL		LuaMsgBox(lua_State * Lua)
	{
		MsgBox("%s", (char*)lua_tostring(Lua, 1));
		return true;
	};
