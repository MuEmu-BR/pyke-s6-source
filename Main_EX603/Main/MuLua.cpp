...
bool MULua::Generic_Call(const char* func, const char* sig, ...)
{
	...
	while (*sig)
	{
		...
		default:
		{
			gConsole.AddMessage(5,"luacall_Generic_Call invalid option (%c)", *(sig - 1));
	...
	{
		gConsole.AddMessage(5,"luacall_Generic_Call error running function '%s': '%s'", func, lua_tolstring(this->m_luaState, -1, 0));
		//_fscanf_l();
		//fscanf()
		if (this->m_bUseSync == true)
		{
