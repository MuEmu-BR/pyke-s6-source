...
void EntryProc(HINSTANCE hins) // OK
{
	...
	if(GetLastError() == ERROR_ALREADY_EXISTS)
	{
		...
		GetPrivateProfileString("AntihackServer","CustomerName","",CustomerName,sizeof(CustomerName),path);

		EncDecKey1 = GetPrivateProfileInt("AntihackServer","EncDecKey1",0,path);

		EncDecKey2 = GetPrivateProfileInt("AntihackServer","EncDecKey2",0,path);

		InternalVersion = GetPrivateProfileInt("AntihackServer","InternalVersion",0,path);
