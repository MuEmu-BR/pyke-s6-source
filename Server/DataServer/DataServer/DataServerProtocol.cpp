...
void DataServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0xFB:
			...
			{
			case 0x00:
				DGAchievementsLoad((PMSG_GDREQ_ACH_LOAD*)lpMsg, index);
				break;
			case 0x01:
				GDAchievementsSave((PMSG_GDREQ_ACH_SAVE*)lpMsg, index);
				break;
			}
			break;
        case 0xF7:
...
void GDCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg,int index) // OK
{
	...
	else
	{
		...
		pMsg.Zen = (DWORD)gQueryManager.GetAsInteger("Zen");

		pMsg.ChaosBank = (DWORD)gQueryManager.GetAsInteger("Chaos");

		pMsg.BlessBank = (DWORD)gQueryManager.GetAsInteger("Bless");

		pMsg.SoulBank = (DWORD)gQueryManager.GetAsInteger("Souls");

		pMsg.LifeBank = (DWORD)gQueryManager.GetAsInteger("Lifes");

		pMsg.CreateonBank = (DWORD)gQueryManager.GetAsInteger("Creations");

		pMsg.GuardianBank = (DWORD)gQueryManager.GetAsInteger("Guardians");

		pMsg.HarmonyBank = (DWORD)gQueryManager.GetAsInteger("Harmonys");

		pMsg.LowStoneBank = (DWORD)gQueryManager.GetAsInteger("LowStones");

		pMsg.HighStoneBank = (DWORD)gQueryManager.GetAsInteger("HighStones");

		pMsg.GemStoneBank = (DWORD)gQueryManager.GetAsInteger("GemStones");

		pMsg.TheGift	= gQueryManager.GetAsInteger("TheGift"); // only add in is void

		pMsg.GensExitTick = gQueryManager.GetAsInteger("GensExitTime");
		...
		{
			pMsg.ExWQuestStart[0] = gQueryManager.GetAsInteger("ExQuestStart_1");
			pMsg.ExWQuestNum[0] = gQueryManager.GetAsInteger("ExQuestNum_1");
			pMsg.ExWQuestCount[0][0] = gQueryManager.GetAsInteger("ExQuestCount1_1");
			pMsg.ExWQuestCount[0][1] = gQueryManager.GetAsInteger("ExQuestCount2_1");
			pMsg.ExWQuestCount[0][2] = gQueryManager.GetAsInteger("ExQuestCount3_1");
			pMsg.ExWQuestCount[0][3] = gQueryManager.GetAsInteger("ExQuestCount4_1");
			pMsg.ExWQuestCount[0][4] = gQueryManager.GetAsInteger("ExQuestCount5_1");

			pMsg.ExWQuestStart[1] = gQueryManager.GetAsInteger("ExQuestStart_2");
			pMsg.ExWQuestNum[1] = gQueryManager.GetAsInteger("ExQuestNum_2");
			pMsg.ExWQuestCount[1][0] = gQueryManager.GetAsInteger("ExQuestCount1_2");
			pMsg.ExWQuestCount[1][1] = gQueryManager.GetAsInteger("ExQuestCount2_2");
			pMsg.ExWQuestCount[1][2] = gQueryManager.GetAsInteger("ExQuestCount3_2");
			pMsg.ExWQuestCount[1][3] = gQueryManager.GetAsInteger("ExQuestCount4_2");
			pMsg.ExWQuestCount[1][4] = gQueryManager.GetAsInteger("ExQuestCount5_2");

			pMsg.ExWQuestStart[2] = gQueryManager.GetAsInteger("ExQuestStart_3");
			pMsg.ExWQuestNum[2] = gQueryManager.GetAsInteger("ExQuestNum_3");
			pMsg.ExWQuestCount[2][0] = gQueryManager.GetAsInteger("ExQuestCount1_3");
			pMsg.ExWQuestCount[2][1] = gQueryManager.GetAsInteger("ExQuestCount2_3");
			pMsg.ExWQuestCount[2][2] = gQueryManager.GetAsInteger("ExQuestCount3_3");
			pMsg.ExWQuestCount[2][3] = gQueryManager.GetAsInteger("ExQuestCount4_3");
			pMsg.ExWQuestCount[2][4] = gQueryManager.GetAsInteger("ExQuestCount5_3");
...
void GDCharacterInfoSaveRecv(SDHP_CHARACTER_INFO_SAVE_RECV* lpMsg) // OK
{
	...
	gQueryManager.ExecQuery("UPDATE Character SET cLevel=%d,Class=%d,LevelUpPoint=%d,Experience=%d,Strength=%d,Dexterity=%d,Vitality=%d,Energy=%d,Leadership=%d,Inventory=?,MagicList=?,Money=%d,Life=%f,MaxLife=%f,Mana=%f,MaxMana=%f,BP=%f,MaxBP=%f,Shield=%f,MaxShield=%f,MapNumber=%d,MapPosX=%d,MapPosY=%d,MapDir=%d,PkCount=%d,PkLevel=%d,PkTime=%d,Quest=?,EffectList=?,FruitAddPoint=%d,FruitSubPoint=%d,ExtInventory=%d,GensExitTime=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->Level, lpMsg->Class, lpMsg->LevelUpPoint, lpMsg->Experience, lpMsg->Strength, lpMsg->Dexterity, lpMsg->Vitality, lpMsg->Energy, lpMsg->Leadership, lpMsg->Money, (float)lpMsg->Life, (float)lpMsg->MaxLife, (float)lpMsg->Mana, (float)lpMsg->MaxMana, (float)lpMsg->BP, (float)lpMsg->MaxBP, (float)lpMsg->Shield, (float)lpMsg->MaxShield, lpMsg->Map, lpMsg->X, lpMsg->Y, lpMsg->Dir, lpMsg->PKCount, lpMsg->PKLevel, lpMsg->PKTime, lpMsg->FruitAddPoint, lpMsg->FruitSubPoint, lpMsg->ExtInventory, lpMsg->GensExitTick, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
	...
	gQueryManager.ExecQuery("UPDATE Character SET Zen=%d, Chaos=%d, Bless=%d, Souls=%d, Lifes=%d, Creations=%d, Guardians=%d, Harmonys=%d, LowStones=%d, HighStones=%d, GemStones=%d WHERE Name='%s'", lpMsg->Zen, lpMsg->ChaosBank, lpMsg->BlessBank, lpMsg->SoulBank, lpMsg->LifeBank, lpMsg->CreateonBank, lpMsg->GuardianBank, lpMsg->HarmonyBank, lpMsg->LowStoneBank, lpMsg->HighStoneBank, lpMsg->GemStoneBank, lpMsg->name);
	gQueryManager.Close();

	gQueryManager.ExecQuery("UPDATE QuestSystem SET ExQuestStart_1=%d, ExQuestNum_1=%d, ExQuestCount1_1=%d, ExQuestCount2_1=%d, ExQuestCount3_1=%d, ExQuestCount4_1=%d, ExQuestCount5_1=%d, ExQuestStart_2=%d, ExQuestNum_2=%d, ExQuestCount1_2=%d, ExQuestCount2_2=%d, ExQuestCount3_2=%d, ExQuestCount4_2=%d, ExQuestCount5_2=%d, ExQuestStart_3=%d, ExQuestNum_3=%d, ExQuestCount1_3=%d, ExQuestCount2_3=%d, ExQuestCount3_3=%d, ExQuestCount4_3=%d, ExQuestCount5_3=%d WHERE Name = '%s'",
...
void DGAchievementsLoad(PMSG_GDREQ_ACH_LOAD* lpMsg, int aIndex)
{
	...
	if (gQueryManager.ExecQuery("EXEC ACHIEVEMENTS_LOAD '%s'", lpMsg->szName) == FALSE)
	{
		gQueryManager.Close();
		pMsg.bResult = false;
	}
	...
	if (pMsg.bResult)
	{
		...
		{
			char text[20];
			sprintf(text, "count_%d", i + 1);
			pMsg.counter[i] = gQueryManager.GetAsInteger(text);
		}
		gQueryManager.Close();
	}
...
{
	gQueryManager.ExecQuery("EXEC ACHIEVEMENTS_SAVE '%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
		lpMsg->szName,
		lpMsg->counter[0],
		lpMsg->counter[1],
		lpMsg->counter[2],
