...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	switch(head)
	{
		...
		case 0x91:
			...
			break;
		case 0x95:
			gGoldenArcher.CGRegisterRecv((PMSG_GOLDEN_ARCHER_REGISTER_RECV*)lpMsg, aIndex);
			break;
		case 0x9A:
			gBloodCastle.CGBloodCastleEnterRecv((PMSG_BLOOD_CASTLE_ENTER_RECV*)lpMsg,aIndex);
			break;
		case 0x9D:
			gGoldenArcher.CGSerialRecv((PMSG_GOLDEN_ARCHER_SERIAL_RECV*)lpMsg, aIndex);
			break;
		case 0xA0:
...
void CGResetStatsRecv(PMSG_RESET_STATS_RECV* lpMsg, int aIndex)
{
	...
	if (lpMsg->Type == 0)
	{
		if (lpObj->Coin1 < gServerInfo.ResetPointWC[lpObj->AccountLevel])
		{
		...
		{
			NewPoints += lpObj->Leadership - ((g_ResetSystem.m_BonusCommand*lpObj->Reset));
		}
		...
		lpObj->Leadership = gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].Leadership + (g_ResetSystem.m_BonusCommand*lpObj->Reset);
		...
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, gServerInfo.ResetPointWC[lpObj->AccountLevel], 0, 0,0);
	...
	if (lpMsg->Type == 1)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(871));
			return;
		}
	...
	if (lpMsg->Type == 2)
	{
		if (lpObj->Coin1 < ((DWORD)gServerInfo.ResetMasterWC[lpObj->AccountLevel]))
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(870));
			return;
		}
		...
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, gServerInfo.ResetMasterWC[lpObj->AccountLevel], 0, 0,0);
	}
...
void CGWarehouseOpen(PMSG_WAREHOUSEINTERFACE_RECV* lpMsg, int aIndex)
{
	...
	/*if(number < 0 || number >= gServerInfo.m_CustomWarehouseMaxNumber[lpObj->AccountLevel])
	{
