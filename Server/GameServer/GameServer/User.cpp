...
//	lpObj->AttackCustomOfflineMoneyDelay = 0;
	...
	memset(&lpObj->ach, 0, sizeof(lpObj->ach));

	for (int i = 0; i<ACHIEVEMENTS_COUNT; i++)
	{
		lpObj->ach.counter[i] = 0;
	}
	...
	lpObj->SoulBank = 0;
	lpObj->LifeBank = 0;
	lpObj->CreateonBank = 0;
	lpObj->GuardianBank = 0;
	lpObj->HarmonyBank = 0;
	lpObj->LowStoneBank = 0;
	lpObj->HighStoneBank = 0;
	lpObj->GemStoneBank = 0;
}
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	gAchievements.PlayerKill(lpTarget->Index, lpObj->Index);
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				if(lpObj->GensExitTick > 0)
				{
					...
					if(lpObj->GensExitTickSec > 60)
					{
						...
						{
							lpObj->GensExitTick = 0;
							gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(865));
					}
				}
			}
...
BOOL gObjGuildWarProc(GUILD_INFO_STRUCT * lpGuild1, GUILD_INFO_STRUCT * lpGuild2, int score)
{
	...
	for(n = 0; n < MAX_GUILD_USER; n++)
	{
		if(lpGuild1->Use[n] && lpGuild1->Index[n] >= 0)
		{
			...
			gAchievements.GuildWarWin(lpGuild1->Index[n]);
		}
	}
