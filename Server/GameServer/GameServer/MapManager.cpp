...
void CMapManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.PKSystem = lpMemScript->GetAsNumber();

			info.OffTradeEnable = lpMemScript->GetAsNumber();
...
int CMapManager::PKSystemEnable(int index) // OK
{
	std::map<int, MAP_MANAGER_INFO>::iterator it = this->m_MapManagerInfo.find(index);
	...
	{
		return ((it->second.PKSystem == -1) ? 0 : it->second.PKSystem);
	}
}
