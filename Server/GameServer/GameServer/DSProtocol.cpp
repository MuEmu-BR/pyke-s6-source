...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0xFB:
			...
			{
			case 0x00:
				gAchievements.DGPlayerLoad((PMSG_DGANS_ACH_LOAD*)lpMsg);
				break;
			}
			break;
		case 0xF7:
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	gAchievements.GCSettingsData(lpObj->Index);
	...
	gJewelsBank.EnvioJewelsBank(lpObj->Index);
...
void GDCreateItemSend(int aIndex, BYTE map,BYTE x,BYTE y,int index,BYTE level,BYTE dur,BYTE Option1,BYTE Option2,BYTE Option3,int LootIndex,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus,DWORD duration) // OK
{
	...
	if (map == 0xFF)
	{
		gAchievements.SuccessChaosMix(aIndex);
	}
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	pMsg.Zen = (DWORD)lpObj->Zen;
	pMsg.ChaosBank = (DWORD)lpObj->ChaosBank;
	pMsg.BlessBank = (DWORD)lpObj->BlessBank;
	pMsg.SoulBank = (DWORD)lpObj->SoulBank;
	pMsg.LifeBank = (DWORD)lpObj->LifeBank;
	pMsg.CreateonBank = (DWORD)lpObj->CreateonBank;
	pMsg.GuardianBank = (DWORD)lpObj->GuardianBank;
	pMsg.HarmonyBank = (DWORD)lpObj->HarmonyBank;
	pMsg.LowStoneBank = (DWORD)lpObj->LowStoneBank;
	pMsg.HighStoneBank = (DWORD)lpObj->HighStoneBank;
	pMsg.GemStoneBank = (DWORD)lpObj->GemStoneBank;
	...
	{
		pMsg.ExWQuestStart[a] = lpObj->ExWQuestStart[a];
		pMsg.ExWQuestNum[a] = lpObj->ExWQuestNum[a];

		for (int m = 0; m < EW_MAX_MISSION; m++)
		{
			pMsg.ExWQuestCount[a][m] = lpObj->ExWQuestCount[a][m];
		}
	}
	...
	gAchievements.GDPlayerSave(aIndex);
