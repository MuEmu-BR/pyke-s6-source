...
void CServerInfo::ReadCustomInfo() // OK
{
	...
	gCustomArcher.Load(gPath.GetFullPath("Custom\\CustomArcher.xml"));
...
//---------- cosas de pet
	...
	gEventTimer.Load(gPath.GetFullPath("Custom\\EventTimer\\EventTimer.txt"));
	...
	gPKSystem.Load(gPath.GetFullPath("Custom\\PKSystem.xml"));
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		{
			g_ResetSystem.SendResetData(&gObj[n], false);
		}
	}
...
void CServerInfo::ReadSkillInfo() // OK
{
	...
	gCustomDisableSkill.Load(gPath.GetFullPath("Skill\\DisableSkill.txt"));
...
void CServerInfo::ReloadBotInfo() // OK
{
	...
	ObjBotBuff.MakeBot();
#endif
	BotAlchemist.Load(gPath.GetFullPath("Custom\\BotSystem\\BotAlchemist.txt"));
	BotAlchemist.MakeBot();

	BotTrader.Load(gPath.GetFullPath("Custom\\BotSystem\\BotTrader.txt"));
	BotTrader.MakeBot();
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_AntihackCRC = GetPrivateProfileInt(section, "AntihackDllCRC", 0, path);
	...
	GetPrivateProfileString("GameServerInfo", "GuardMessage", 0, this->GuardMessage, sizeof(this->GuardMessage), path);
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	this->m_CustomArcherSwitch = GetPrivateProfileInt(section, "CustomArcherSwitch", 0, path);

	this->m_CustomArcherEnable[0] = GetPrivateProfileInt(section, "CustomArcherEnable_AL0 ", 0, path);

	this->m_CustomArcherEnable[1] = GetPrivateProfileInt(section, "CustomArcherEnable_AL1 ", 0, path);

	this->m_CustomArcherEnable[2] = GetPrivateProfileInt(section, "CustomArcherEnable_AL2 ", 0, path);

	this->m_CustomArcherEnable[3] = GetPrivateProfileInt(section, "CustomArcherEnable_AL3", 0, path);
	...
	this->MoveEffect[0] = GetPrivateProfileInt(section, "MoveEffect_AL0", 0, path);

	this->MoveEffect[1] = GetPrivateProfileInt(section, "MoveEffect_AL1", 0, path);

	this->MoveEffect[2] = GetPrivateProfileInt(section, "MoveEffect_AL2", 0, path);

	this->MoveEffect[3] = GetPrivateProfileInt(section, "MoveEffect_AL3", 0, path);
