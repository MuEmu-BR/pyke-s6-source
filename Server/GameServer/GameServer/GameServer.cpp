...
LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) // OK
{
	...
	switch(message)
	{
...

		{
			...
			wsprintf(text, "GameServer %s - Version %s ", GAMESERVER_NAME, GAMESERVER_VERSION);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				case IDM_STARTQUIZ:
					...
					break;
#if(SKY_EVENT == 1)
				case IDM_STARTSKYEVENT:
					gSkyEvent.StartEvent();
					break;
#endif
#if(SWAMP_EVENT == 1)
				case IDM_EVENTS_SWAMP_EVENT:
					gSwampEvent.StartEvent();
					break;
#endif
#if(HAPPY_HOUR == 1)
				case IDM_STARTHAPPYHOURSNORMAL:
					ObjHappy.StartNormalHappyHours();
					break;
				case IDM_STARTHAPPYHOURSMASTER:
					ObjMasterH.StartMasterHours();
					break;
#endif
				case IDM_STARTDROP:
